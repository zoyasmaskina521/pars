...
In the opened browser window (local operator or remote worker through VNC):
...
### 5.1) Validate saved session before daemon start
Use this command to verify the persisted session is reusable before switching back to headless service:

```bash
npm run check-session
```

Exit code `0` means `authenticated`. Non-zero means session is not usable yet (`challenge_detected`, `auth_expired`, or `down`).
