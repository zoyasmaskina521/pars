...
import { checkSavedSession, runManualSessionBootstrap } from './auth/browser.js';
...
async function main() {
  ...
  if (args.has('--bootstrap-session')) {
    const status = await runManualSessionBootstrap(env, logger);
    logger.info({ status, sessionStatePath: env.SESSION_STATE_PATH }, 'Bootstrap session finished');
    process.exitCode = status === 'success' ? 0 : 2;
    return;
  }

  if (args.has('--check-session')) {
    const status = await checkSavedSession(env, logger);
    logger.info({ status, sessionStatePath: env.SESSION_STATE_PATH }, 'Saved session check finished');
    process.exitCode = status === 'authenticated' ? 0 : 2;
    return;
  }
