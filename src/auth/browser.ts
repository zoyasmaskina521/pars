...
export type SessionCheckStatus = 'authenticated' | 'auth_expired' | 'challenge_detected' | 'down';
...
export async function checkSavedSession(env: Env, logger: pino.Logger): Promise<SessionCheckStatus> {
  const { browser, context, page } = await createSession(env, logger);
  try {
    const state = await detectSiteOrChallenge(page, env.SITE_URL);
    if (state === 'down') return 'down';
    if (state === 'challenge') return 'challenge_detected';

    const authState = await ensureAuthenticated(page, env, logger);
    if (authState === 'authenticated') return 'authenticated';
    if (authState === 'challenge') return 'challenge_detected';
    return 'auth_expired';
  } finally {
    await context.close().catch(() => undefined);
    await browser.close().catch(() => undefined);
